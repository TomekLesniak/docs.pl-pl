...
* Język C# jest prosty, wydajny, bezpieczny dla typów i zorientowany obiektowo, zachowując wyrazistości i elegancji języków w stylu języka C. Każda osoba znająca język C i podobne Języki odnajduje kilka problemów w dostosowywaniu do języka C#. Aby dowiedzieć się więcej o języku C#, zapoznaj się z [przewodnikiem w języku c#](../csharp/index.yml) .
...
W poniższym przykładzie środowisko uruchomieniowe zgłasza <xref:System.IndexOutOfRangeException> wyjątek w celu wymuszenia bezpieczeństwa pamięci:
...
Niektóre obiekty odwołują się do *niezarządzanych zasobów*. Zasoby niezarządzane to zasoby, które nie są automatycznie obsługiwane przez środowisko uruchomieniowe platformy .NET. Na przykład dojście do pliku jest zasobem niezarządzanym. <xref:System.IO.FileStream>Obiekt jest obiektem zarządzanym, ale odwołuje się do dojścia do pliku, który jest niezarządzany. Gdy skończysz korzystać z programu <xref:System.IO.FileStream> , musisz zwolnić dojście do pliku.

W programie .NET obiekty, które odwołują się do zasobów niezarządzanych, implementują <xref:System.IDisposable> interfejs. Gdy skończysz korzystać z obiektu, wywołasz <xref:System.IDisposable.Dispose> metodę obiektu, która jest odpowiedzialna za zwolnienie niezarządzanych zasobów. Języki .NET zapewniają wygodną [ `using` instrukcję](../csharp/language-reference/keywords/using.md) dotyczącą takich obiektów, jak pokazano w następującym przykładzie:

[!code-csharp[UnmanagedResources](../../samples/snippets/csharp/snippets/tour/UnmanagedResources.csx#L1-L6)]

Po `using` zakończeniu bloku środowisko uruchomieniowe platformy .NET automatycznie wywołuje `stream` <xref:System.IDisposable.Dispose> metodę obiektu, która zwalnia dojście do pliku. Środowisko uruchomieniowe robi to również wtedy, gdy wyjątek powoduje opuszczenie bloku.

Aby uzyskać więcej informacji, zobacz następujące tematy:

* W języku C# Zobacz temat [using (odwołanie w C#)](../csharp/language-reference/keywords/using-statement.md) .
* W przypadku języka F # zobacz [Zarządzanie zasobami: słowo kluczowe use](../fsharp/language-reference/resource-management-the-use-keyword.md).
...
Delegaty są podobne do wskaźników funkcji języka C++, z tą różnicą, że są typu bezpieczne. Są one rodzajową metodą rozłączoną w systemie typów CLR. Regularne metody są dołączane do klasy i można je bezpośrednio wywołać za pomocą konwencji wywoływania statycznego lub wystąpienia.
...
LINQ to zaawansowany zestaw funkcji dla języków C# i Visual Basic, które umożliwiają pisanie prostego, deklaratywnego kodu do obsługi danych. Dane mogą znajdować się w wielu formularzach (takich jak obiekty znajdujące się w pamięci, bazy danych SQL lub dokumenty XML), ale kod LINQ, który jest zwykle nie różni się od źródła danych.

Aby dowiedzieć się więcej i zapoznać się z przykładami, zobacz artykuł [Omówienie programu LINQ (Language Integrated Query)](./linq/index.md) .
...
Poniższy przykład jest zmodyfikowaną wersją `ToString()` metody z `StringBuilder` klasy. Ilustruje to, jak użycie `unsafe` kodu może efektywnie zaimplementować algorytm przez bezpośrednie przechodzenie między fragmentami pamięci:

[!code-csharp[Unsafe](../../samples/snippets/csharp/snippets/tour/Unsafe.csx)]

## <a name="next-steps"></a>Następne kroki

Jeśli interesuje Cię Przewodnik po funkcjach języka C#, zapoznaj się [z samouczkiem języka c#](../csharp/tour-of-csharp/index.md).

Jeśli interesuje Cię Przewodnik po funkcjach języka F #, zobacz [Przewodnik po języku f #](../fsharp/tour.md).
