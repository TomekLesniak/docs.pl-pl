...
Za pomocą lokalnego magazynu wątków (TLS) można przechowywać dane, które są unikatowe dla wątku i domeny aplikacji. Platforma .NET oferuje dwa sposoby korzystania z zarządzanej TLS: pól statycznych względnych dla wątków i miejsc danych.  
...
Platforma .NET zapewnia dynamiczne miejsca danych, które są unikatowe dla kombinacji wątków i domen aplikacji. Istnieją dwa typy miejsc danych: nazwane gniazda i nienazwane gniazda. Oba są implementowane przy użyciu <xref:System.LocalDataStoreSlot> struktury.  
