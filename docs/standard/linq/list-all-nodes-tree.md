...
---
# <a name="how-to-list-all-nodes-in-a-tree-linq-to-xml"></a>Jak wyświetlić listę wszystkich węzłów w drzewie (LINQ to XML)

Czasami warto wyświetlić listę wszystkich węzłów w drzewie — na przykład, aby precyzyjnie dowiedzieć się, jak metoda lub właściwość wpływa na drzewo. W poniższym przykładzie dla języka C# i Visual Basic są wyświetlane węzły drzewa. Każdy węzeł jest reprezentowany przez wyrażenie XPath, które określa lokalizację węzła w drzewie.

> [!NOTE]
> Nie jest szczególnie przydatne do wykonywania wyrażeń XPath przy użyciu LINQ to XML. Zapytania LINQ to XML wykonują lepsze niż wyrażenia XPath i są znacznie bardziej wydajne. Jednak jako sposób identyfikowania węzłów w drzewie XML, wyrażenie XPath działa prawidłowo.

## <a name="example-use-xpath-to-list-all-nodes-in-a-tree"></a>Przykład: Użyj XPath, aby wyświetlić listę wszystkich węzłów w drzewie

Ten przykład zawiera funkcję o nazwie `GetXPath` , która generuje określone wyrażenie XPath dla dowolnego węzła w drzewie XML. Robi to nawet wtedy, gdy węzły znajdują się w przestrzeni nazw. Elementy w wygenerowanych wyrażeniach XPath mają prefiksy przestrzeni nazw zgodnie z wymaganiami.

Przykład najpierw tworzy małe drzewo XML zawierające kilka typów węzłów. Następnie wykonuje iterację w węzłach i drukuje wyrażenie XPath dla każdego węzła.

Deklaracja XML nie jest węzłem drzewa.

Oto drzewo XML, które służy jako dane wejściowe:
...
Poniżej znajduje się lista węzłów w drzewie XML, zgodnie z definicją w wyrażeniach XPath:
