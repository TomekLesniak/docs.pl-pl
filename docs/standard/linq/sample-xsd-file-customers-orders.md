...
---
# <a name="sample-xsd-file-customers-and-orders-linq-to-xml"></a>Przykładowy plik XSD: klienci i zamówienia (LINQ to XML)

Następujący plik XSD jest używany w różnych przykładach w dokumentacji LINQ to XML. Ten plik zawiera definicję schematu dla [przykładowego pliku XML: klienci i zamówienia](sample-xml-file-customers-orders.md). Schemat używa `xs:key` i `xs:keyref` funkcji XSD, aby określić, że `CustomerID` atrybut `Customer` elementu jest kluczem i aby ustanowić relację między `CustomerID` elementem w każdym `Order` elemencie a `CustomerID` atrybutem w każdym `Customer` elemencie.
