...
---
# <a name="how-to-perform-streaming-transform-of-large-xml-documents-linq-to-xml"></a>Jak przeprowadzić transformację strumieniową dużych dokumentów XML (LINQ to XML)
...
Techniki przesyłania strumieniowego najlepiej zastosować w sytuacjach, gdy trzeba przetwarzać dokument źródłowy tylko raz i można przetwarzać elementy w kolejności dokumentu. Niektóre standardowe operatory zapytań, takie jak <xref:System.Linq.Enumerable.OrderBy%2A> , iteruje źródło, zbierają wszystkie dane, sortują je, a następnie zwracają pierwszy element w sekwencji. Należy pamiętać, że jeśli używasz operatora zapytania, który materializuje jego źródło przed uzyskaniem pierwszego elementu, nie zachowasz małego rozmiaru pamięci dla aplikacji.

Nawet jeśli używasz techniki opisanej w artykule [jak przesyłać fragmenty XML z dostępem do informacji nagłówka](stream-xml-fragments-access-header-information.md), jeśli próbujesz utworzyć drzewo XML zawierające przekształcony dokument, użycie pamięci będzie zbyt duże.

Istnieją dwa główne podejścia. Jednym z metod jest użycie odroczonych charakterystyk przetwarzania <xref:System.Xml.Linq.XStreamingElement> . Innym rozwiązaniem jest utworzenie <xref:System.Xml.XmlWriter> i użycie możliwości LINQ to XML do zapisywania elementów w <xref:System.Xml.XmlWriter> . W tym artykule przedstawiono oba podejścia.

## <a name="example-use-the-deferred-execution-capabilities-of-xstreamingelement-to-stream-the-output"></a>Przykład: Użyj funkcji odroczonego wykonywania w `XStreamingElement` celu strumieniowego przesyłania danych wyjściowych

Poniższy przykład jest oparty na przykładzie w [metodzie przesyłania strumieniowego XML z dostępem do informacji nagłówka](stream-xml-fragments-access-header-information.md).
...
## <a name="example-use-linq-to-xml-to-write-elements-to-an-xmlwriter"></a>Przykład: Użyj LINQ to XML, aby pisać elementy do XmlWriter

Poniższy przykład jest również oparty na przykładzie w [metodzie przesyłania strumieniowego XML z dostępem do informacji nagłówka](stream-xml-fragments-access-header-information.md).

Ten przykład używa możliwości LINQ to XML do zapisywania elementów w <xref:System.Xml.XmlWriter> . W tym przykładzie można przekształcić bardzo duży dokument przy zachowaniu małej ilości pamięci.
...
Ten przykład używa tego samego dokumentu źródłowego, Source.xml, jak w poprzednim przykładzie. Generuje również dokładnie te same dane wyjściowe.

Użycie <xref:System.Xml.Linq.XStreamingElement> do przesyłania strumieniowego wyjściowego pliku XML jest preferowany do zapisu w <xref:System.Xml.XmlWriter> .
