...
---
# <a name="how-to-project-an-object-graph-linq-to-xml"></a>Jak projektować Graf obiektów (LINQ to XML)

Możesz użyć języka C# lub Visual Basic do projektu lub wypełniania grafu obiektów na podstawie kodu XML.

## <a name="example-populate-an-object-graph-from-xml"></a>Przykład: wypełnianie grafu obiektów z pliku XML

Ten przykład wypełnia wykres obiektu za pomocą `Address` `PurchaseOrder` klasy,, i `PurchaseOrderItem` z [przykładowego pliku XML dokumentu XML: typowe zamówienie zakupu](sample-xml-file-typical-purchase-order.md).
...
Wynik zapytania LINQ jest zwracany jako <xref:System.Collections.Generic.IEnumerable%601> z `PurchaseOrderItem` . Elementy w `PurchaseOrder` klasie są typu <xref:System.Collections.Generic.IEnumerable%601> `PurchaseOrderItem` . Kod używa <xref:System.Linq.Enumerable.ToList%2A> metody rozszerzającej, aby utworzyć <xref:System.Collections.Generic.List%601> kolekcję na podstawie wyników zapytania.
