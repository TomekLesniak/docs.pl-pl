...
---
# <a name="refactor-using-a-pure-function-linq-to-xml"></a>Refaktoryzacja przy użyciu czystej funkcji (LINQ to XML)

Poniższy przykład refaktoryzacji poprzedni przykład, [Refaktoryzacja przy użyciu metody rozszerzenia](refactor-extension-method.md), do używania czystej funkcji. Przenosi kod, który znajduje tekst akapitu do czystej metody statycznej `ParagraphText` .

## <a name="example-retrieve-the-paragraph-nodes-and-their-styles"></a>Przykład: pobieranie węzłów akapitu i ich stylów

Ten przykład przetwarza dokument WordprocessingML, pobierając węzły akapitu. Identyfikuje także styl każdego akapitu. Ten przykład kompiluje się zgodnie z poprzednimi przykładami w tym samouczku. Kod refaktoryzacji jest wywoływany w komentarzach w kodzie poniżej.
...
W następnym artykule w tym samouczku pokazano, jak projektować XML w innym kształcie:

- [Projekt XML w innym kształcie](project-xml-different-shape.md)
