...
description: Ten plik XML — który jest używany w przykładach — zawiera dane dotyczące klientów i zamówień.
ms.date: 07/20/2015
...
ms.locfileid: "89553603"
---
# <a name="sample-xml-file-customers-and-orders-linq-to-xml"></a><span data-ttu-id="6d0bc-103">Przykładowy plik XML: klienci i zamówienia (LINQ to XML)</span><span class="sxs-lookup"><span data-stu-id="6d0bc-103">Sample XML file: Customers and orders (LINQ to XML)</span></span>

<span data-ttu-id="6d0bc-104">Poniższy plik XML jest używany w różnych przykładach w dokumentacji LINQ to XML.</span><span class="sxs-lookup"><span data-stu-id="6d0bc-104">The following XML file is used in various examples in the LINQ to XML documentation.</span></span> <span data-ttu-id="6d0bc-105">Ten plik zawiera klientów i zamówienia.</span><span class="sxs-lookup"><span data-stu-id="6d0bc-105">This file contains customers and orders.</span></span>

<span data-ttu-id="6d0bc-106">[Przykładowy plik XSD artykułu: klienci i zamówienia](sample-xsd-file-customers-orders.md) zawierają XSD, których można użyć do zweryfikowania tego dokumentu.</span><span class="sxs-lookup"><span data-stu-id="6d0bc-106">The article [Sample XSD file: Customers and orders](sample-xsd-file-customers-orders.md) contains an XSD that can be used to validate this document.</span></span> <span data-ttu-id="6d0bc-107">Używa `xs:key` i `xs:keyref` funkcji XSD, aby określić, że `CustomerID` atrybut `Customer` elementu jest kluczem i aby ustanowić relację między `CustomerID` elementem w każdym `Order` elemencie a `CustomerID` atrybutem w każdym `Customer` elemencie.</span><span class="sxs-lookup"><span data-stu-id="6d0bc-107">It uses the `xs:key` and `xs:keyref` features of XSD to establish that the `CustomerID` attribute of the `Customer` element is a key, and to establish a relationship between the `CustomerID` element in each `Order` element and the `CustomerID` attribute in each `Customer` element.</span></span>

<span data-ttu-id="6d0bc-108">Przykład pisania zapytań LINQ, które wykorzystują tę relację przy użyciu `Join` klauzuli, można znaleźć w temacie [jak sprzęgać dwie kolekcje](join-two-collections.md).</span><span class="sxs-lookup"><span data-stu-id="6d0bc-108">For an example of writing LINQ queries that take advantage of this relationship using the `Join` clause, see [How to join two collections](join-two-collections.md).</span></span>

## <a name="customersordersxml"></a><span data-ttu-id="6d0bc-109">CustomersOrders.xml</span><span class="sxs-lookup"><span data-stu-id="6d0bc-109">CustomersOrders.xml</span></span>
