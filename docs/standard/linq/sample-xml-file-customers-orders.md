...
Poniższy plik XML jest używany w różnych przykładach w dokumentacji LINQ to XML. Ten plik zawiera klientów i zamówienia.

[Przykładowy plik XSD artykułu: klienci i zamówienia](sample-xsd-file-customers-orders.md) zawierają XSD, których można użyć do zweryfikowania tego dokumentu. Używa `xs:key` i `xs:keyref` funkcji XSD, aby określić, że `CustomerID` atrybut `Customer` elementu jest kluczem i aby ustanowić relację między `CustomerID` elementem w każdym `Order` elemencie a `CustomerID` atrybutem w każdym `Customer` elemencie.

Przykład pisania zapytań LINQ, które wykorzystują tę relację przy użyciu `Join` klauzuli, można znaleźć w temacie [jak sprzęgać dwie kolekcje](join-two-collections.md).
