...
|--------------------|-------------|-----------------------------------|  
|Węzeł tekstowy pasuje do obu `xsl:strip-space` i `xsl:preserve-space` .|3.4|Odzyskiwanie|  
|Węzeł źródłowy jest zgodny z więcej niż jedną regułą szablonu.|5,5|Odzyskiwanie|  
|Identyfikator URI przestrzeni nazw jest zadeklarowany jako alias dla wielu identyfikatorów URI przestrzeni nazw, a wszystkie mają ten sam priorytet importu.|ppkt|Odzyskiwanie|  
|`name`Atrybut w `xsl:attribute` i `xsl:element` wygenerowany na podstawie wartości atrybutu nie jest QName.|7.1.2, 7.1.3|Porn|  
|Dwa zestawy atrybutów o tej samej imporcie i rozwiniętej nazwie mają wspólny atrybut i nie istnieje żaden inny zestaw atrybutów zawierający wspólny atrybut o tej samej nazwie o wyższym znaczeniu.|7.1.4|Odzyskiwanie|  
...
|Wynik tworzenia wystąpienia zawartości `xsl:processing-instruction` zawiera ciąg "? >"|7.3|Odzyskiwanie|  
|Wynik tworzenia wystąpienia zawartości `xsl:processing-instruction` zawiera ciąg "--" lub kończący się znakiem "-".|7,4|Odzyskiwanie|  
...
|Istnieje wiele atrybutów o tej samej nazwie, ale różne wartości, które nie są nazwanymi elementami Section w ramach tego `xsl:output` samego pierwszeństwa importu.|16|Odzyskiwanie|  
|Procesor nie obsługuje kodowania w `xsl:output` atrybucie kodowania.|16,1|Odzyskiwanie|  
...
|`value`Atrybut `xsl:number` jest NaN, nieskończony lub mniejszy niż 0,5|Errata 24|Odzyskiwanie|  
|Drugi argument jest ustawiony na funkcję dokumentu, a odwołanie do identyfikatora URI jest względne.|Errata 14|Odzyskiwanie|  
