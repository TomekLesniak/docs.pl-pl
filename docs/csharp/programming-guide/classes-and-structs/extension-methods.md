...
`WordCount`Metodę rozszerzenia można wprowadzić do zakresu przy użyciu tej `using` dyrektywy:
...
`MethodB`Metoda rozszerzenia nigdy nie jest wywoływana, ponieważ jej nazwa i podpis są dokładnie zgodne z metodami już zaimplementowanymi przez klasy.
...
- Metody rozszerzenia są włączane do zakresu na poziomie przestrzeni nazw. Na przykład jeśli masz wiele klas statycznych, które zawierają metody rozszerzające w pojedynczej przestrzeni nazw o nazwie `Extensions` , wszystkie te elementy zostaną wprowadzone do zakresu przez `using Extensions;` dyrektywę.
