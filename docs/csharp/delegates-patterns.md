...
`Where`Metoda używa delegata, który określa, które elementy sekwencji przechodzą filtr. Podczas tworzenia zapytania LINQ należy podać implementację delegata w tym konkretnym celu.
...
Następnie Dodajmy kilka argumentów do `LogMessage()` metody, aby Klasa dzienników tworzyła bardziej strukturalne komunikaty:
...
Na koniec zaktualizujmy metodę LogMessage, tak aby była niezawodna dla tych przypadków, gdy nie wybrano żadnego mechanizmu wyjściowego. Bieżąca implementacja spowoduje zgłoszenie, `NullReferenceException` gdy delegat nie `WriteMessage` ma dołączonej listy wywołań.
Możesz preferować projekt, który jest dyskretnie kontynuowany, gdy nie dołączono żadnych metod. Jest to proste użycie operatora warunkowego null, połączonego z `Delegate.Invoke()` metodą:
...
Dwuobwodowy operator warunkowy o wartości null ( `?.` ), gdy lewy operand ( `WriteMessage` w tym przypadku) ma wartość null, co oznacza, że nie podjęto próby zarejestrowania komunikatu.

Nie można znaleźć `Invoke()` metody wymienionej w dokumentacji dla `System.Delegate` lub `System.MulticastDelegate` . Kompilator generuje bezpieczną `Invoke` metodę typu dla dowolnego zadeklarowanego typu delegata. W tym przykładzie oznacza to, że `Invoke` przyjmuje jeden `string` argument i ma zwracany typ void.
