...
W tym prostym przykładzie można zobaczyć, że wiele typów obejmuje tworzenie i pracę z drzewami wyrażeń. Taka złożoność jest niezbędna do zapewnienia możliwości rozbudowanego słownictwa zapewnianego przez język C#.

## <a name="navigating-the-apis"></a>Nawigowanie po interfejsach API

Istnieją typy węzłów wyrażeń, które mapują do niemal wszystkich elementów składni języka C#. Każdy typ ma określone metody dla tego typu elementu języka. Jest to bardzo dużo do utrzymania w Twoim głowie. Zamiast próbować znają wszystko, Oto techniki używane do pracy z drzewami wyrażeń:
