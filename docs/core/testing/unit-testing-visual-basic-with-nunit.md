...
W katalogu *Test Unit-VB-nunit* Uruchom `dotnet test` ponownie. `dotnet test`Polecenie uruchamia kompilację dla `PrimeService` projektu, a następnie dla `PrimeService.Tests` projektu. Po skompilowaniu obu projektów jest uruchamiany ten pojedynczy test. Przekazuje.
