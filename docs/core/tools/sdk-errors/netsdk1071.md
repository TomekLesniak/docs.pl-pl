...
4. Nie należy jawnie dodawać ani aktualizować odwołań do `Microsoft.NETCore.App` ani `NETSTandard.Library` w projektach .NET Framework. Pakiet NuGet automatycznie instaluje dowolną wersję `NETStandard.Library` potrzebną w przypadku korzystania z pakietu NuGet opartego na .NET Standard.
