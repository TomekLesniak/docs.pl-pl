...
- Jeśli musisz obsługiwać `Encoding.GetEncoding("utf-7", ...)` :
  ...
  Można ponownie włączyć obsługę tego programu za pośrednictwem przełącznika zgodności. Ten przełącznik zgodności można określić w pliku *. csproj* aplikacji lub w [pliku konfiguracji czasu wykonywania](../../../run-time-config/index.md), jak pokazano w poniższych przykładach.
