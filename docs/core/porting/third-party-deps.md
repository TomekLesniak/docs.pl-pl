...
ms.locfileid: "92223611"
---
# <a name="analyze-your-dependencies-to-port-code-to-net-core"></a>Analizowanie zależności w kodzie portu do programu .NET Core

Aby przenieść kod do programu .NET Core lub .NET Standard, musisz zrozumieć zależności. Zależności zewnętrzne to pakiety NuGet lub pliki, do `.dll` których odwołuje się w projekcie, ale nie są one kompilowane.

## <a name="migrate-your-nuget-packages-to-packagereference"></a>Migrowanie pakietów NuGet do programu `PackageReference`

Program .NET Core używa [PackageReference](/nuget/consume-packages/package-references-in-project-files) , aby określić zależności pakietów. Jeśli używasz [packages.config](/nuget/reference/packages-config) , aby określić pakiety w projekcie, przekonwertuj je do `PackageReference` formatu, ponieważ `packages.config` nie jest to obsługiwane w programie .NET Core.

Aby dowiedzieć się, jak przeprowadzić migrację, zobacz artykuł [Migrowanie z packages.config do PackageReference](/nuget/reference/migrate-packages-config-to-package-reference) .

## <a name="upgrade-your-nuget-packages"></a>Uaktualnianie pakietów NuGet

Po przeprowadzeniu migracji projektu do `PackageReference` formatu Sprawdź, czy pakiety są zgodne z platformą .NET Core.
...
> [!IMPORTANT]
> Podczas TFMs, który obsługuje pakiet, należy pamiętać, że `netcoreapp*` chociaż jest zgodny, jest przeznaczony tylko dla projektów .NET Core, a nie dla projektów .NET Standard.
> Biblioteka przeznaczona tylko dla elementów docelowych `netcoreapp*` i nie `netstandard*` może być używana przez inne aplikacje platformy .NET Core.
...
Aby pominąć ostrzeżenie przez edytowanie pliku projektu, Znajdź `PackageReference` wpis dla pakietu, dla którego chcesz pominąć ostrzeżenie, i Dodaj `NoWarn` atrybut. Ten `NoWarn` atrybut akceptuje listę wszystkich identyfikatorów ostrzeżeń rozdzielanych przecinkami. Poniższy przykład pokazuje, jak pominąć `NU1701` Ostrzeżenie dla `Huitian.PowerCollections` pakietu, edytując plik projektu ręcznie:
...
Zespół platformy .NET chce wiedzieć, które biblioteki są najważniejsze do obsługi platformy .NET Core. Możesz wysłać wiadomość e-mail do dotnet@microsoft.com informacji o bibliotekach, których chcesz użyć.
