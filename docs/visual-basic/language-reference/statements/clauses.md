---
title: Klauzule
ms.date: 07/20/2015
helpviewer_keywords:
- clauses [Visual Basic]
ms.assetid: 5e89f5f3-d129-4ed1-8857-30cd71cff95e
ms.openlocfilehash: 0acfc61476389e6dd3266c6de73889f54e77585e
ms.sourcegitcommit: d2db216e46323f73b32ae312c9e4135258e5d68e
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 09/22/2020
ms.locfileid: "90865944"
---
# <a name="clauses-visual-basic"></a>Klauzule (Visual Basic)

Tematy w tej sekcji dokumentu Visual Basic klauzule czasu wykonywania.  
  
## <a name="in-this-section"></a>W tej sekcji  

 [Użyj](alias-clause.md)  
  
 [Definicj](as-clause.md)  
  
 [Handles](handles-clause.md)  
  
 [Implementuje](implements-clause.md)  
  
 [W](in-clause.md)  
  
 [Przekształca](into-clause.md)  
  
 [Z](of-clause.md)  
  
## <a name="related-sections"></a>Sekcje pokrewne  

 [Dokumentacja języka Visual Basic](../index.md)  
