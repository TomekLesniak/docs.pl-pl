...
- Nieprawidłowe wywołanie platformy lub deklaracje COM, które są używane `Integer` do obsługi zamiast <xref:System.IntPtr> .  
...
- Użycie wywołania platformy lub międzyoperacyjności modelu COM ze składnikami, które nie istnieją na wszystkich platformach.  
 ...
 Opcja **-platform** ogranicza niektóre problemy, Jeśli wiesz, że masz założeń dotyczących architektury, w której kod będzie uruchamiany. W szczególności:  
...
- Zestawy skompilowane z programem `-platform:x86` będą wykonywane na 32-bitowym CLR uruchomionym w emulatorze WOW64.  
  
- Pliki wykonywalne skompilowane z użyciem `-platform:anycpu` zostaną wykonane na 64-bitowym środowisku CLR.  
...
- Pliki wykonywalne, które są kompilowane przy użyciu, `-platform:anycpu32bitpreferred` zostaną wykonane na 32-bitowym CLR.  
...
## <a name="example"></a>Przykład  
 ...
 Poniższy przykład ilustruje sposób użycia `-platform` opcji kompilatora.  
