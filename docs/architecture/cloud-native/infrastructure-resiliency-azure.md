...
## <a name="design-with-resiliency"></a>Projektowanie z odpornością

Mamy sprawność, że aplikacja reaguje na awarie i nadal pozostaje funkcjonalna. Dokument oficjalny dokument dotyczący [odporności na platformie Azure](https://azure.microsoft.com/mediahandler/files/resourcefiles/resilience-in-azure-whitepaper/Resilience%20in%20Azure.pdf)zawiera wskazówki dotyczące osiągania odporności na platformę Azure. Oto kilka najważniejszych zaleceń:

- *Awaria sprzętowa.* Tworzenie nadmiarowości w aplikacji przez wdrożenie składników w różnych domenach błędów. Na przykład upewnij się, że maszyny wirtualne platformy Azure są umieszczone w różnych stojakach przy użyciu zestawów dostępności.

- *Błąd centrum danych.* Tworzenie nadmiarowości w aplikacji ze strefami izolacji błędów w centrach danych. Na przykład upewnij się, że maszyny wirtualne platformy Azure są umieszczone w różnych odizolowanych centrach danych przy użyciu Strefy dostępności platformy Azure.

- *Awaria regionalna.* Replikowanie danych i składników do innego regionu, dzięki czemu aplikacje mogą być szybko odzyskiwane. Na przykład użyj Azure Site Recovery do replikowania maszyn wirtualnych platformy Azure do innego regionu platformy Azure.

- *Duże obciążenie.* Równoważ obciążenie między wystąpieniami, aby obsłużyć maksymalne użycie. Na przykład Umieść co najmniej dwie maszyny wirtualne platformy Azure za modułem równoważenia obciążenia, aby dystrybuować ruch do wszystkich maszyn wirtualnych.

- *Przypadkowe usuwanie lub uszkodzenie danych.* Utwórz kopię zapasową danych, aby można ją było przywrócić w przypadku usunięcia lub uszkodzenia. Na przykład użyj Azure Backup, aby okresowo tworzyć kopie zapasowe maszyn wirtualnych platformy Azure.
...
**Rysunek 6-5**. AKS i Traffic Manager platformy Azure
...
> [Dalej](resilient-communications.md)
