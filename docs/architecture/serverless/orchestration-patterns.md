...
Poniższy kod stanowi uproszczony przykład sposobu, w jaki `ProcessPayment` Metoda może wyglądać następująco:
...
W tym scenariuszu `DurableOrchestrationClient` można użyć możliwości sprawdzenia stanu uruchomionego przepływu pracy. Przy użyciu `HttpTrigger` do uruchomienia przepływu pracy, `CreateCheckStatusResponse` Metoda może być używana do zwracania wystąpienia `HttpResponseMessage` . Ta odpowiedź zapewnia klientowi identyfikator URI w ładunku, którego można użyć do sprawdzenia stanu uruchomionego procesu.
...
`DurableOrchestrationContext``CreateTimer`Metoda ustawia harmonogram następnego wywołania pętli w celu sprawdzenia, czy zmieniają się ceny giełdowe. `DurableOrchestrationContext` ma również `CurrentUtcDateTime` Właściwość, aby uzyskać bieżącą wartość DateTime w formacie UTC. Lepiej jest używać tej właściwości zamiast `DateTime.UtcNow` ponieważ jest ona łatwo zastosowana do testowania.
