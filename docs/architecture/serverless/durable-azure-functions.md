...
Funkcje programu Orchestrator nie mogą używać powiązań innych niż OrchestrationTriggerAttribute. Tego atrybutu można używać tylko z typem parametru DurableOrchestrationContext. Nie można używać innych danych wejściowych, ponieważ deserializacja wejść w sygnaturze funkcji nie jest obsługiwana. Aby uzyskać dane wejściowe dostarczone przez klienta aranżacji, \<T\> należy użyć metody getinput.
...
Wiele wystąpień aranżacji można uruchomić i uruchomić w tym samym czasie. Wywołanie `StartNewAsync` metody na `DurableOrchestrationClient` uruchamia nowe wystąpienie aranżacji. Metoda zwraca `Task<string>` , która kończy się po rozpoczęciu aranżacji. Wyjątek typu jest `TimeoutException` zgłaszany, jeśli aranżacja nie została uruchomiona w ciągu 30 sekund.
