...
W związku z tym po skompilowaniu kontenerów aplikacji w potoku CI należy również wdrożyć, zintegrować i przetestować aplikację jako całość ze wszystkimi kontenerami w ramach hosta Integration Docker lub nawet do klastra testowego, do którego są dystrybuowane kontenery.
...
Zazwyczaj można chcieć mieć prywatne repozytoria dla obrazów niestandardowych w repozytorium prywatnym w Azure Container Registry lub w rejestrze lokalnym, takim jak zaufany rejestr platformy Docker, lub w rejestrze chmury publicznej z ograniczonym dostępem (na przykład w przypadku usługi Docker Hub), chociaż w tym ostatnim przypadku kod nie jest typu "open source", należy zaufać zabezpieczeniom dostawcy. W obu przypadkach stosowana Metoda jest podobna i opiera się na `docker push` poleceniu, jak pokazano na rysunku 5-4.
...
Niezmienności obrazów platformy Docker zapewnia powtarzające się wdrożenie z możliwościami, które zostały opracowane, przetestowane przez CI i działają w środowisku produkcyjnym. Po opublikowaniu obrazów platformy Docker w rejestrze platformy Docker (prywatnym lub publicznym) można wdrożyć je w kilku środowiskach, które mogą mieć (produkcja, pytania i odpowiedzi, przemieszczanie itp.) z potoku dysku CD przy użyciu zadań potoku Azure DevOps Services lub Azure DevOps Services Release Management.

Jednak w tym momencie zależy od rodzaju wdrażanej aplikacji platformy Docker. Wdrażanie prostej aplikacji (z punktu widzenia składu i wdrożenia), takiego jak aplikacja monolityczna składająca się z kilku kontenerów lub usług i wdrożona na kilku serwerach lub maszynach wirtualnych, różni się od wdrażania bardziej złożonej aplikacji, takiej jak aplikacja zorientowana na mikrousługi z możliwościami skalowania. Te dwa scenariusze zostały omówione w poniższych sekcjach.
...
Podczas tworzenia wydania w Azure DevOps Services pobiera on zestaw artefaktów wejściowych. Te artefakty są przeznaczone do niezmiennego okresu istnienia wersji we wszystkich środowiskach. Gdy wprowadzisz kontenery, artefakty wejściowe identyfikują obrazy w rejestrze do wdrożenia. W zależności od tego, jak te obrazy są identyfikowane, nie ma gwarancji, że przez cały czas trwania wydania nie są one tak samo, jak w przypadku odwołania do `myimage:latest` `docker-compose` pliku.
...
Za pomocą szablonów Azure DevOps Services można utworzyć nowy obraz, opublikować go w rejestrze platformy Docker, uruchomić go na hostach z systemem Linux lub Windows i użyć poleceń, takich jak `docker-compose` wdrożenie wielu kontenerów jako całej aplikacji, za pomocą Azure DevOps Services Release Management możliwości dla wielu środowisk, jak pokazano na rysunku 5-8.
...
Należy jednak pamiętać, że scenariusz przedstawiony na rysunku 5-6 i zaimplementowany na rysunku 5-8 jest prostym (jest wdrażany na jednym hoście platformy Docker i maszynach wirtualnych) i prawdopodobnie powinien być używany tylko na potrzeby scenariuszy tworzenia i testowania. W większości scenariuszy produkcyjnych przedsiębiorstwa należy zapewnić wysoką dostępność (HA) i łatwą w zarządzaniu skalowalność, równoważyć obciążenie między wieloma węzłami, serwerami i maszynami wirtualnymi, a także "inteligentną pracę awaryjną", więc jeśli serwer lub węzeł ulegnie awarii, jego usługi i kontenery zostaną przeniesione na inny serwer hosta lub maszynę wirtualną. W takim przypadku potrzebne są bardziej zaawansowane technologie, takie jak klastry kontenerów, Koordynatory i harmonogramy. W ten sposób można wdrożyć w tych klastrach, obsługując zaawansowane scenariusze opisane w następnej sekcji.
...
Z punktu widzenia dysku CD, a Azure DevOps Services w szczególnie, można uruchomić specjalnie wykonane zadania wdrażania ze środowisk Azure DevOps Services Release Management, które będą wdrażać aplikacje kontenerów w klastrach rozproszonych w usłudze Container Service, jak pokazano na rysunku 5-9.
...
Początkowo podczas wdrażania do określonych klastrów lub Orchestrator można tradycyjnie używać konkretnych skryptów i mechanizmów wdrażania dla każdego koordynatora (to jest, Kubernetes i Service Fabric mają różne mechanizmy wdrażania) zamiast prostszego i łatwego w użyciu `docker-compose` narzędzia opartego na `docker-compose.yml` pliku definicji. Jednak dzięki obsłudze Azure DevOps Services narzędzia Docker Deploy pokazano na rysunku 5-10, teraz można również wdrożyć je do obsługiwanych koordynatorów, używając tylko znanego `docker-compose.yml` pliku, ponieważ narzędzie wykonuje "tłumaczenie" (z `docker-compose.yml` pliku do formatu wymaganego przez program Orchestrator).
...
Ponieważ aplikacje i zarządzanie nimi na poziomie przedsiębiorstwa są głównym podmiotem w ramach i dla siebie, a ze względu na typ operacji i osób pracujących na tym poziomie (operacje IT), a także duży zakres tego obszaru, cały następny rozdział jest zaważniejszy do wyjaśnienia.
