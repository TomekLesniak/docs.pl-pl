...
Aby dodać dane do bazy danych podczas uruchamiania aplikacji, można dodać kod podobny do poniższego do `Main` metody w `Program` klasie projektu interfejsu API sieci Web:
...
W eShopOnContainers, `basket-api` mikrousługa korzysta z pamięci podręcznej Redis działającej jako kontener. Ten `basketdata` kontener jest definiowany jako część pliku *Docker-Compose. yml* z wielokontenerem, jak pokazano w następującym przykładzie:
...
Ten kod w Docker-Compose. yml definiuje kontener o nazwie `basketdata` opartej na obrazie Redis i wewnętrznie publikuje port 6379. Oznacza to, że będzie dostępny tylko z innych kontenerów uruchomionych w ramach hosta platformy Docker.

Na koniec w pliku *Docker-Compose. override. yml* , `basket-api` mikrousługa dla przykładu eShopOnContainers definiuje parametry połączenia do użycia dla tego kontenera Redis:
...
Jak wspomniano wcześniej, nazwa mikrousługi `basketdata` jest rozpoznawana przez system DNS w sieci wewnętrznej platformy Docker.
