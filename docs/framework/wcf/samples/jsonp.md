...
```
 ...
 Strona sieci Web może wywoływać usługę WCF REST, ponieważ usługa korzysta z funkcji <xref:System.ServiceModel.Description.WebScriptEndpoint> with z `crossDomainScriptAccessEnabled` ustawioną na `true` . Obie te konfiguracje są wykonywane w pliku Web.config w ramach \<system.serviceModel> elementu.
...
```
 ...
 Element ScriptManager zarządza interakcją z usługą i ukrywa złożoność ręcznej implementacji dostępu JSONP. Gdy `crossDomainScriptAccessEnabled` jest ustawiona na, `true` a format odpowiedzi dla operacji to JSON, Infrastruktura WCF bada identyfikator URI żądania dla parametru ciągu zapytania wywołania zwrotnego i ZAWIJA odpowiedź JSON przy użyciu wartości parametru ciągu zapytania wywołania zwrotnego. W przykładzie Strona sieci Web wywołuje usługę WCF REST z następującym identyfikatorem URI.
