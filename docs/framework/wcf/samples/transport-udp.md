...
- Dupleks (IDuplexChannel)  
 ...
 Każdy z tych MEPs może również obsługiwać sesje. Dodatkowe funkcje udostępniane przez kanał obsługujący sesje polegają na tym, że są skorelowane wszystkie komunikaty wysyłane i odbierane w kanale. Wzorzec Request-Response jest autonomiczną sesją dwustronicową, ponieważ żądanie i odpowiedź są skorelowane. W przeciwieństwie do wzorca Request-Response, który obsługuje sesje, oznacza, że wszystkie pary żądanie/odpowiedź w tym kanale są skorelowane ze sobą. Dzięki temu można uzyskać łącznie sześć MEPs — datagram, żądanie-odpowiedź, dupleks, datagram z sesjami, Request-Response z sesjami i dupleks z sesjami — do wyboru.  
...
```  
 ...
 Ponieważ niestandardowe elementy powiązania transportu są odpowiedzialne za obsługę adresowania, `IPolicyExportExtension` implementacja na serwerze `UdpTransportBindingElement` musi również obsługiwać eksportowanie odpowiednich potwierdzeń zasad WS-Addressing, aby wskazać używaną wersję WS-Addressing.  
