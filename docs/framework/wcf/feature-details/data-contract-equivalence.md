...
> Należy zauważyć, że nazwy kontraktu danych i przestrzenie nazw, a także nazwy składowych danych, w których jest rozróżniana wielkość liter.  
 ...
 Aby uzyskać więcej informacji na temat nazw kontraktów danych i przestrzeni nazw, a także nazw elementów członkowskich danych, zobacz [nazwy kontraktów danych](data-contract-names.md).  
...
## <a name="inheritance-interfaces-and-data-contract-equivalence"></a>Dziedziczenie, interfejsy i równoważność kontraktu danych  
 ...
 Podczas przekazywania parametrów i zwracanych wartości między klientem a usługą nie można wysyłać kontraktu danych z klasy bazowej, gdy punkt końcowy otrzymywania oczekuje kontraktu danych z klasy pochodnej. Jest to zgodne z założenia programowanie zorientowane obiektowo. W poprzednim przykładzie obiekt typu `Person` nie może być wysyłany, gdy `Employee` jest oczekiwany.  
  
 Kontrakt danych z klasy pochodnej może być wysyłany, gdy oczekiwano kontraktu danych z klasy bazowej, ale tylko wtedy, gdy punkt końcowy odbioru "wie" typu pochodnego przy użyciu <xref:System.Runtime.Serialization.KnownTypeAttribute> . Aby uzyskać więcej informacji, zobacz [znane typy kontraktu danych](data-contract-known-types.md). W poprzednim przykładzie obiekt typu `Employee` może być wysyłany `Person` , gdy jest oczekiwany, ale tylko wtedy, gdy kod odbiorcy wykorzystuje <xref:System.Runtime.Serialization.KnownTypeAttribute> do dołączenia do listy znanych typów.  
  
 W przypadku przekazywania parametrów i zwracanych wartości między aplikacjami, jeśli oczekiwany typ jest interfejsem, jest równoważne oczekiwanemu typowi typu <xref:System.Object> . Ze względu na to, że każdy typ ostatecznie pochodzi od <xref:System.Object> , każdy kontrakt danych ostatecznie pochodzi z kontraktu danych dla <xref:System.Object> . W ten sposób każdy typ kontraktu danych może zostać przesłany, gdy oczekiwany jest interfejs. Dodatkowe kroki są wymagane do pomyślnej pracy z interfejsami; Aby uzyskać więcej informacji, zobacz [znane typy kontraktu danych](data-contract-known-types.md).  
