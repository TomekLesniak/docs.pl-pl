...
W protokole SOAP 1,2 wprowadzono opcjonalny parametr akcji dla `application/soap+xml` typu nośnika. Ten parametr jest przydatny do optymalizowania wysyłania komunikatów bez konieczności analizowania treści komunikatu protokołu SOAP, gdy WS-Addressing nie jest używany.
...
Gdy WS-Addressing jest wyłączone, a żądanie przychodzące nie zawiera parametru akcji, komunikat `Action` jest traktowany jako nieokreślony.
...
Wszystkie wersje WS-Addressing, do których usługa WCF implementuje odwołania do punktów końcowych, do opisywania punkty końcowe.
...
Na przykład, jeśli punkt końcowy WCF implementuje protokół WS-ReliableMessaging, `AcksTo` nagłówek zwracany przez taki punkt końcowy `CreateSequenceResponse` używa wersji WS-Addressing, którą `EncodingBinding` element określa dla tego punktu końcowego.
...
B3121: Funkcja WCF korzysta z mechanizmów opisanych w sekcji specyfikacji WS-MetadataExchange (MEX) 6, aby uwzględnić metadane dla odwołań punktów końcowych przez wartość lub przez odwołanie.
...
W przypadku obu wersji WS-Addressing WCF używa następujących nagłówków komunikatów zgodnie ze specyfikacją,, `wsa:To` , `wsa:ReplyTo` `wsa:Action` `wsa:MessageID` i `wsa:RelatesTo` .

B3211: dla wszystkich wersji WS-Addressing, WCF honoruje, ale nie wygenerowało pola, WS-Addressing nagłówki wiadomości `wsa:FaultTo` i `wsa:From` .
...
- R3311: Obiekt żądający musi zawierać `wsa:To` `wsa:Action` nagłówki, i i dla wszystkich parametrów referencyjnych określonych przez odwołanie do punktu końcowego. Gdy zostanie użyta WS-Addressing 2004/08 i [właściwości odwołania] są określone przez odwołanie do punktu końcowego, odpowiednie nagłówki muszą zostać dodane do komunikatu.
...
Gdy punkt końcowy programu WCF jest skonfigurowany dla komunikatu z danym `Action` do przestrzegania wzorca żądanie-odpowiedź, punkt końcowy WCF postępuje zgodnie z zachowaniem i wymaganiami poniżej. O ile nie określono inaczej, zachowania i reguły mają zastosowanie w przypadku obu wersji WS-Addressing obsługiwanych w programie WCF:
...
- R3322: gdy WS-Addressing 2004/08 jest używany, `ReplyTo` również musi być uwzględniony w żądaniu.

- R3323: gdy WS-Addressing 1,0 jest używany i `ReplyTo` nie występuje w żądaniu, zostanie użyte domyślne odwołanie do punktu końcowego z właściwością [address] równą `http://www.w3.org/2005/08/addressing/anonymous` .
...
### <a name="wsdl-11-binding-and-ws-policy-assertions"></a>Powiązania WSDL 1,1 i potwierdzeń WS-Policy

#### <a name="indicating-use-of-ws-addressing"></a>Wskazywanie użycia WS-Addressing
...
`wsaw10:UsingAddressing`Element jest popożyczony z [WS-Addressing-WSDL] i jest używany w kontekście WS-Policy zgodności z tą specyfikacją w sekcji 3.1.2.
...
Gdy WS-Security jest używany do ochrony integralności komunikatów, WS-Addressing nagłówki wiadomości, a także nagłówki, które wynikają z parametrów odwołania lub właściwości (lub oba), muszą być podpisane razem z treścią wiadomości.

### <a name="examples"></a>Przykłady

#### <a name="one-way-message"></a>Komunikat One-Way

W tym scenariuszu nadawca wysyła wiadomość jednokierunkową do odbiorcy. Używane są protokoły SOAP 1,2, HTTP 1,1 i W3C WS-Addressing 1,0.
...
4. Utwórz kopertę protokołu SOAP XOP, która jest kopią oryginalnej koperty protokołu SOAP, ale z elementami podrzędnymi każdego elementu informacji o elemencie zidentyfikowanym w poprzednim kroku zamienionym przez `xop:Include` element informacji o elemencie utworzony w następujący sposób:
    ...
    5. Generowanie nowej binarnej części MIME zawierającej zawartość utworzoną przez dane binarne, zdekodowaną od zamienionych znaków przetworzonych jako Base64, Content-ID nagłówka z 4B, Content-Transfer-Encoding header z 4C, nagłówek Content-Type w przypadku generowania w kroku 4D.
...
- Pełny formularz Backus-Naur (BNF) nagłówka Content-Type dla komunikatów MIME jest wymieniony w dokumencie RFC 2045, sekcja 5,1.
...
### <a name="ws-policy-assertion-for-mtom"></a>WS-Policy potwierdzenie dla MTOM
...
MTOM jest mechanizmem kodowania podobnym do `text/xml` kodu XML danych binarnych WCF. MTOM oferuje naturalną kompozycję przy użyciu WS-Security i innych protokołów WS-*: komunikat zabezpieczony za pomocą WS-Security można zoptymalizować za pomocą mechanizmu MTOM.
