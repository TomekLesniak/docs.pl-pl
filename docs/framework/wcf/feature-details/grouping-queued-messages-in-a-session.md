...
4. Każda operacja usługi wymaga transakcji. Określ ten <xref:System.ServiceModel.OperationBehaviorAttribute> atrybut. Operacja, która kończy transakcję, powinna również mieć ustawioną wartość <xref:System.ServiceModel.OperationBehaviorAttribute.TransactionAutoComplete> `true` .  
  
    ```csharp  
    [OperationBehavior(TransactionScopeRequired = true, TransactionAutoComplete = true)]
    ```  
  
5. Skonfiguruj punkt końcowy, który używa powiązania dostarczonego przez system `NetMsmqBinding` .  
  
6. Utwórz kolejkę transakcyjną przy użyciu <xref:System.Messaging> . Możesz również utworzyć kolejkę za pomocą usługi kolejkowania komunikatów (MSMQ) lub MMC. W takim przypadku należy utworzyć kolejkę transakcyjną.  
