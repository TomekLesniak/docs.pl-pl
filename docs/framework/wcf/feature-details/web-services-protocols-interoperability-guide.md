...
Protokoły infrastruktury współdziałania są udostępniane przez specyfikacje WS-*. Kanały WCF zapewniają obsługę wielu protokołów usługi WS- \* Infrastructure. Kanały WCF są konfigurowane przy użyciu elementów powiązania. Poniższe tabele zawierają pełną listę protokołów WS- \* Infrastructure wdrożonych przez różne elementy powiązania WCF.
...
|Specyfikacja/dokument|Łącze|
|-----------------------------|----------|
|XML|[XML (XML) 1,0 (czwarta wersja)](https://www.w3.org/TR/REC-xml/)|
