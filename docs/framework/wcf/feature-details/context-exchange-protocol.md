...
## <a name="mode-2-application-context-management"></a>Tryb 2: zarządzanie kontekstem aplikacji  
 ...
 Wartość kontekstu musi być chroniona przed modyfikacją podczas przesyłania z tego samego powodu, WS-Addressing nagłówki są chronione — nagłówek jest używany do określenia miejsca, w którym ma zostać wysłane żądanie do usługi. W `wsc:Context` związku z tym nagłówek musi być cyfrowo podpisany lub podpisany i zaszyfrowany na poziomie protokołu SOAP lub transportu, gdy powiązanie oferuje funkcję ochrony wiadomości. Gdy pliki cookie protokołu HTTP są używane do propagowania kontekstu, powinny być chronione przy użyciu zabezpieczeń transportu.  
