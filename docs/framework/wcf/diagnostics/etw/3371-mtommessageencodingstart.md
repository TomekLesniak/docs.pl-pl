---
title: 3371 — MtomMessageEncodingStart
ms.date: 03/30/2017
ms.assetid: b1ba417f-f765-4567-82e3-ea2080e0ac73
ms.openlocfilehash: f66aa1f4c9117fe10076c972e74bb9f87717f0ae
ms.sourcegitcommit: bc293b14af795e0e999e3304dd40c0222cf2ffe4
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 11/26/2020
ms.locfileid: "96259645"
---
# <a name="3371---mtommessageencodingstart"></a>3371 — MtomMessageEncodingStart

## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID (Identyfikator)|3371|  
|Słowa kluczowe|Kanał|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows-Application Server — aplikacje/debugowanie|  
  
## <a name="description"></a>Opis  

 To zdarzenie jest emitowane po rozpoczęciu kodowania komunikatu przez obiekt mtommessageencoder rozpoczął.  
  
## <a name="message"></a>Wiadomość  

 Obiekt mtommessageencoder rozpoczął rozpoczął kodowanie komunikatu.  
  
## <a name="details"></a>Szczegóły
