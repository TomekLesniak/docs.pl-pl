---
title: 3373 — BinaryMessageDecodingStart
ms.date: 03/30/2017
ms.assetid: 20b13b33-d67d-47fd-aeec-a7fa3fc87c26
ms.openlocfilehash: d43039072b2363eaa0c821becafe2f649dff4dfa
ms.sourcegitcommit: bc293b14af795e0e999e3304dd40c0222cf2ffe4
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 11/26/2020
ms.locfileid: "96259658"
---
# <a name="3373---binarymessagedecodingstart"></a>3373 — BinaryMessageDecodingStart

## <a name="properties"></a>Właściwości  
  
|||  
|-|-|  
|ID (Identyfikator)|3373|  
|Słowa kluczowe|Kanał|  
|Poziom|Pełny|  
|Kanał|Microsoft-Windows-Application Server — aplikacje/debugowanie|  
  
## <a name="description"></a>Opis  

 To zdarzenie jest emitowane po rozpoczęciu dekodowania komunikatu przez obiekt BinaryMessageEncoder rozpoczął.  
  
## <a name="message"></a>Wiadomość  

 Obiekt BinaryMessageEncoder rozpoczął rozpoczął dekodowanie komunikatu.  
  
## <a name="details"></a>Szczegóły
