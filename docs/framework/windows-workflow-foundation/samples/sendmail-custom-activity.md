...
|EnableSsl|bool|Określa, czy program <xref:System.Net.Mail.SmtpClient> używa SSL (SSL) do szyfrowania połączenia.|  
|Nazwa użytkownika|Ciąg|Nazwa użytkownika, aby skonfigurować poświadczenia w celu uwierzytelnienia <xref:System.Net.Mail.SmtpClient.Credentials%2A> Właściwości nadawcy.|  
|Hasło|Ciąg|Hasło w celu skonfigurowania poświadczeń w celu uwierzytelnienia <xref:System.Net.Mail.SmtpClient.Credentials%2A> Właściwości nadawcy.|  
|Temat|<xref:System.Activities.InArgument%601>\<string>|Temat wiadomości.|  
|Treść|<xref:System.Activities.InArgument%601>\<string>|Treść wiadomości.|  
|Załączniki|<xref:System.Activities.InArgument%601>\<string>|Kolekcja załączników służąca do przechowywania danych dołączonych do tej wiadomości e-mail.|  
|Źródło|<xref:System.Net.Mail.MailAddress>|Adres od dla tej wiadomości e-mail.|  
|Działanie|<xref:System.Activities.InArgument%601>\<<xref:System.Net.Mail.MailAddressCollection>>|Kolekcja adresów zawierająca adresatów tej wiadomości e-mail.|  
...
|Tokeny|<xref:System.Activities.InArgument%601><IDictionary\<string, string>>|Tokeny do zamiany w treści. Ta funkcja umożliwia użytkownikom Określanie niektórych wartości w treści, niż można zastąpić je później za pomocą tokenów dostarczonych przy użyciu tej właściwości.|  
|BodyTemplateFilePath|Ciąg|Ścieżka szablonu dla treści. `SendMail`Działanie kopiuje zawartość tego pliku do jego właściwości Body.<br /><br /> Szablon może zawierać tokeny, które są zastępowane przez zawartość właściwości tokens.|  
...
- [Konfigurowanie usługi SMTP (IIS 6,0)](/previous-versions/windows/it-pro/windows-server-2003/cc784968(v=ws.10))  
  
- [IIS 7,0: Konfigurowanie poczty E-Mail SMTP](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc772058(v=ws.10))  
  
- [Jak zainstalować usługę SMTP](/previous-versions/tn-archive/aa997480(v=exchg.65))  
