...
|Wartość|Opis|  
|-----------|-----------------|  
|*assembly_name*|Prosta nazwa zestawu bez rozszerzenia pliku. Ten atrybut odpowiada <xref:System.Reflection.AssemblyName.Name%2A?displayProperty=nameWithType> właściwości. Na przykład nazwa zestawu o nazwie Extensions.dll to "Extensions". Zobacz sekcję Uwagi, aby uzyskać specjalną formę *assembly_name* , która obsługuje warunkowe dołączanie metadanych z zestawu.|  
...
## <a name="remarks"></a>Uwagi  
 ...
 `<Library>` dyrektywy mogą być warunkowo wykorzystywane. Jeśli nazwa `<Library>` elementu zaczyna się i kończy się gwiazdką ( \* ), `<Library>` dyrektywa ma wpływ tylko wtedy, gdy zestaw określony między gwiazdkami jest przywoływany przez aplikację. Na przykład następująca dyrektywa środowiska uruchomieniowego ma zastosowanie tylko wtedy, gdy aplikacja jest odwołująca się do zestawu Utilities.dll.  
