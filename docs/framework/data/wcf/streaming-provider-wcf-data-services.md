...
Za pomocą Usługi danych programu WCF definiujesz strumień zasobów binarnych przez implementację dostawcy danych przesyłania strumieniowego. Implementacja dostawcy przesyłania strumieniowego dostarcza usługę danych za pomocą strumienia zasobów multimediów skojarzonego z konkretną jednostką jako <xref:System.IO.Stream> obiektem. Ta implementacja umożliwia usłudze danych akceptowanie i zwracanie zasobów multimedialnych za pośrednictwem protokołu HTTP jako strumieni danych binarnych określonego typu MIME.
...
3. Zdefiniuj usługę danych, która implementuje <xref:System.IServiceProvider> interfejs. Usługa danych używa implementacji, <xref:System.IServiceProvider.GetService%2A> Aby uzyskać dostęp do implementacji dostawcy danych przesyłanych strumieniowo. Ta metoda zwraca odpowiednią implementację dostawcy przesyłania strumieniowego.
...
Aby wskazać, że jednostka jest wpisem linku do nośnika, Dodaj `HasStream` atrybut do definicji typu jednostki w modelu koncepcyjnym, jak w poniższym przykładzie:
...
Przykład usługi danych, która korzysta z dostawcy Entity Framework i uwidacznia zasób multimedialny, znajduje się w temacie Post [Data Services Streaming Provider series: implementowanie dostawcy przesyłania strumieniowego (część 1)](/archive/blogs/astoriateam/data-services-streaming-provider-series-implementing-a-streaming-provider-part-1).

**Dostawca odbicia**

Aby wskazać, że jednostka jest wpisem linku do nośnika, należy dodać <xref:System.Data.Services.Common.HasStreamAttribute> do klasy, która definiuje typ jednostki w dostawcy odbicia.

**Niestandardowy dostawca usługi danych**

W przypadku korzystania z niestandardowych dostawców usług należy zaimplementować <xref:System.Data.Services.Providers.IDataServiceMetadataProvider> interfejs w celu zdefiniowania metadanych usługi danych. Aby uzyskać więcej informacji, zobacz [niestandardowe dostawcy usług danych](custom-data-service-providers-wcf-data-services.md). Należy wskazać, że binarny strumień zasobów należy do <xref:System.Data.Services.Providers.ResourceType> obiektu, ustawiając <xref:System.Data.Services.Providers.ResourceType.IsMediaLinkEntry%2A> Właściwość na `true` na <xref:System.Data.Services.Providers.ResourceType> , która reprezentuje typ jednostki, który jest wpisem łącza do nośnika.
...
Aby zapewnić środowisko uruchomieniowe Usługi danych programu WCF z dostępem do <xref:System.Data.Services.Providers.IDataServiceStreamProvider> implementacji, usługa danych, którą tworzysz, musi również zaimplementować <xref:System.IServiceProvider> interfejs. Poniższy przykład pokazuje, jak zaimplementować metodę, <xref:System.IServiceProvider.GetService%2A> Aby zwrócić wystąpienie `PhotoServiceStreamProvider` klasy implementującej <xref:System.Data.Services.Providers.IDataServiceStreamProvider> .
...
Domyślnie program Internet Information Services (IIS) ogranicza także rozmiar żądań do 4 MB. Aby umożliwić usłudze danych otrzymywanie strumieni o rozmiarze większym niż 4 MB podczas uruchamiania w usługach IIS, należy również ustawić `maxRequestLength` atrybut [elementu httpRuntime (schemat ustawień ASP.NET)](/previous-versions/dotnet/netframework-4.0/e1f13641(v=vs.100)) w `<system.web />` sekcji konfiguracji, jak pokazano w następującym przykładzie:
...
- Po odebraniu żądania POST usługa danych wywołuje, <xref:System.Data.Services.Providers.IDataServiceStreamProvider.GetWriteStream%2A> Aby utworzyć zasób multimedialny przed wywołaniem go <xref:System.Data.Services.IUpdatable.SaveChanges%2A> w celu utworzenia wpisu nośnika.

- Implementacja <xref:System.Data.Services.Providers.IDataServiceStreamProvider.GetWriteStream%2A> nie powinna zwracać <xref:System.IO.MemoryStream> obiektu. W przypadku użycia tego rodzaju strumienia problemy z zasobami pamięci będą wykonywane, gdy usługa otrzymuje bardzo duże strumienie danych.
...
- Poniżej przedstawiono kwestie, które należy wziąć pod uwagę podczas przechowywania zasobów multimedialnych w bazie danych programu:
  ...
  - Gdy klient wysyła żądanie POST, aby wstawić wpis nośnika z zasobem multimedialnym w pojedynczym żądaniu, <xref:System.Data.Services.Providers.IDataServiceStreamProvider.GetWriteStream%2A> jest wywoływana w celu uzyskania strumienia, zanim usługa danych umieści nową jednostkę w bazie danych. Implementacja dostawcy przesyłania strumieniowego musi być w stanie obsłużyć to zachowanie usługi danych. Rozważ użycie oddzielnej tabeli danych do przechowywania danych binarnych lub przechowywanie strumienia danych w pliku do momentu wstawienia obiektu do bazy danych.
