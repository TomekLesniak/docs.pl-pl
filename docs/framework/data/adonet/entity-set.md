...
## <a name="example"></a>Przykład  
 ...
 Na poniższym diagramie przedstawiono dwa zestawy jednostek ( `Books` i `Publishers` ) oraz zestaw skojarzeń ( `PublishedBy` ) na podstawie podanego powyżej modelu koncepcyjnego. Usługa BI w `Books` zestawie jednostek reprezentuje wystąpienie `Book` typu jednostki w czasie wykonywania. Podobnie PJ reprezentuje `Publisher` wystąpienie w `Publishers` zestawie jednostek. BiPj reprezentuje wystąpienie `PublishedBy` skojarzenia w `PublishedBy` zestawie skojarzeń.  
