...
Możesz również utworzyć identyfikator logowania mapowany na ten sam certyfikat, a następnie udzielić wszelkich niezbędnych uprawnień na poziomie serwera dla tej nazwy logowania lub dodać nazwę logowania do co najmniej jednej roli serwera stałych. Jest to tak zaprojektowane, aby uniknąć włączania `TRUSTWORTHY` Ustawienia bazy danych dla scenariuszy, w których są konieczne uprawnienia wyższego poziomu.

Po wykonaniu procedury składowanej SQL Server łączy uprawnienia użytkownika certyfikatu i/lub logowania się z osobami wywołującymi. W przeciwieństwie do `EXECUTE AS` klauzuli, nie zmienia kontekstu wykonywania procedury. Wbudowane funkcje, które zwracają nazwy logowania i użytkownika zwracają nazwę obiektu wywołującego, a nie nazwę użytkownika certyfikatu.
...
1. Podpisz procedurę z certyfikatem przy użyciu instrukcji języka Transact-SQL `ADD SIGNATURE TO [procedureName] BY CERTIFICATE [certificateName]` .
...
1. Skopiuj certyfikat do `master` bazy danych programu.

1. Utwórz nazwę logowania skojarzoną z tym certyfikatem przy użyciu instrukcji języka Transact-SQL `CREATE LOGIN [userName] FROM CERTIFICATE [certificateName]` .
...
|Zasób|Opis|
|--------------|-----------------|
|[Podpisywanie modułu](/previous-versions/sql/sql-server-2008/ms345102(v=sql.100))|Opisuje podpisywanie modułu, dostarczając przykładowy scenariusz i linki do odpowiednich artykułów języka Transact-SQL.|
