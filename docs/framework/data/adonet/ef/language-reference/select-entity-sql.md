...
## <a name="remarks"></a>Uwagi  
 ...
 Lista co najmniej jednego wyrażenia zapytania po słowie kluczowym SELECT jest znana jako lista wyboru lub bardziej formalnie jako projekcja. Najbardziej ogólną formą projekcji jest pojedyncze wyrażenie zapytania. Wybranie elementu członkowskiego `member1` z kolekcji `collection1` spowoduje utworzenie nowej kolekcji wszystkich `member1` wartości dla każdego obiektu w `collection1` , jak pokazano w poniższym przykładzie.  
  
```sql  
SELECT collection1.member1 FROM collection1  
```  
  
 Na przykład jeśli `customers` jest kolekcją typu, `Customer` który ma właściwość `Name` typu `string` , wybranie `Name` z spowoduje przekazanie `customers` kolekcji ciągów, jak pokazano w poniższym przykładzie.  
...
## <a name="row-and-value-select-clauses"></a>Klauzule Select z wierszami i wartościami  
 ...
 Każde wyrażenie zapytania w wierszu wybierz musi określać alias. Jeśli alias nie zostanie określony, program [!INCLUDE[esql](../../../../../../includes/esql-md.md)] podejmie próbę wygenerowania aliasu przy użyciu reguł generowania aliasów.  
...
## <a name="differences-from-transact-sql"></a>Różnice w języku Transact-SQL  
 ...
 W przeciwieństwie do języka Transact-SQL, program nie [!INCLUDE[esql](../../../../../../includes/esql-md.md)] obsługuje użycia argumentu * w klauzuli select.  Zamiast tego, program [!INCLUDE[esql](../../../../../../includes/esql-md.md)] umożliwia zaprojektowanie całych rekordów, odwołując się do aliasów kolekcji z klauzuli FROM, jak pokazano w poniższym przykładzie.  
  
```sql  
SELECT * FROM T1, T2  
```  
  
 Poprzednie wyrażenie zapytania Transact-SQL jest wyrażone w [!INCLUDE[esql](../../../../../../includes/esql-md.md)] następujący sposób.  
