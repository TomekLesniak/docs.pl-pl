...
`wsFederationHttpBinding`Kontrolki szczegóły ustawień zabezpieczeń wiadomości w programie [\<message>](message-element-of-wsfederationhttpbinding.md) . Należy zauważyć, że [\<security>](security-of-wsfederationhttpbinding.md) element zapewnia dostęp tylko wtedy, gdy nie można zmienić zabezpieczeń używanych przez powiązanie, po utworzeniu powiązania.
