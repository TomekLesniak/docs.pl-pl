...
|maxReceivedMessageSize|Dodatnia liczba całkowita, która określa maksymalny rozmiar wiadomości (w bajtach), w tym nagłówki, które można odbierać w kanale skonfigurowanym dla tego powiązania. Nadawca wiadomości przekraczającej ten limit spowoduje zwrócenie błędu. Odbiorca odrzuca komunikat i tworzy wpis zdarzenia w dzienniku śledzenia. Wartość domyślna to 65536. **Uwaga:**  Zwiększenie tej samej wartości nie jest wystarczające w trybie zgodności ASP.NET. Należy również zwiększyć wartość `httpRuntime` (zobacz [element httpRuntime (schemat ustawień ASP.NET)](/previous-versions/dotnet/netframework-4.0/e1f13641(v=vs.100))).|  
