...
Edytor lokalizacji zasobów systemu Windows, Winres.exe, to narzędzie układu wizualnego, które pomaga ekspertom w lokalizacji lokalizować Windows Forms zasobów interfejsu użytkownika używanych przez formularze. Pliki resx lub resources używane jako dane wejściowe do Winres.exe mogą być tworzone przy użyciu środowiska projektowania wizualnego, takiego jak Microsoft Visual Studio. Aby uzyskać informacje na temat wdrażania zasobów w aplikacjach .NET Framework, zobacz [zasoby w aplikacjach klasycznych](../resources/index.md).

Winres.exe jest instalowany z programem Visual Studio. Aby uruchomić narzędzie, użyj wiersz polecenia dla deweloperów dla programu Visual Studio. Aby uzyskać więcej informacji, zobacz [wiersza polecenia](developer-command-prompt-for-vs.md).
...
Winres.exe jest aplikacją graficzną, która odtwarza wersję formularza Windows Forms czasu projektowania z samego pliku zasobów, bez dostępu do kodu źródłowego. Winres.exe hostuje **projektanta formularzy** programu Visual Studio Windows Forms i okna **Właściwości** . Te funkcje umożliwiają wizualne edytowanie pliku resources lub resx zawierającego formularz Windows Forms. Zazwyczaj lokalizatory używają Winres.exe do edytowania etykiet kontrolek i dostosowywania lokalizacji i rozmiaru formantów w celu dopasowania etykiet dla kultury docelowej.
...
Należy użyć wersji Winres.exe wydanej przy użyciu .NET Framework. W poniższej tabeli wymieniono zgodne wersje:
...
1. Wpisz `winres` w wierszu polecenia programisty, aby uruchomić Winres.exe.
...
2. Aby otworzyć domyślne zasoby formularza do zlokalizowania, kliknij polecenie **Otwórz** w menu **plik** i przejdź do pliku, aby go otworzyć.
     ...
     Następujące polecenie uruchamia Winres.exe i ładuje formularz skojarzony z programem `TestApp.resx` w projektancie formularzy.

    ```console
    winres TestApp.resx
    ```

     Następujące polecenie uruchamia Winres.exe i ładuje formularz skojarzony z programem `TestApp.resources` w projektancie formularzy.
